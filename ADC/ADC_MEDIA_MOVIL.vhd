...
architecture Behavioral of ADC_MEDIA_MOVIL is
	...
	begin
		...
		D0 : entity work.DISPLAY generic map(BCD => true) port map (BIN => BCD0, D7SEG => HEX0, DP => '0');
		D1 : entity work.DISPLAY generic map(BCD => true) port map (BIN => BCD1, D7SEG => HEX1, DP => '0');
		D2 : entity work.DISPLAY generic map(BCD => true) port map (BIN => BCD2, D7SEG => HEX2, DP => '0');
		D3 : entity work.DISPLAY generic map(BCD => true) port map (BIN => BCD3, D7SEG => HEX3, DP => '1');
